...
class ResponseCache:
    ...
    def similarity(a: str, b: str) -> float:
        ...
        def tokenize(text: str) -> list[str]:
            ...
            ngrams = [text[i:i + 3] for i in range(len(text) - 2)] if len(text) >= 3 else [text]
            return words + ngrams

        tokens_a = tokenize(a_low)
        tokens_b = tokenize(b_low)
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        try:
            val = self._redis.hget(exact_key, "response")
            if val is not None:
                return val, 1.0
        except Exception:
            pass
        ...
        if best_key is not None and best_query is not None and best_score >= self.similarity_threshold:
            if _looks_like_false_hit(query, best_query):
                ...
                return None, best_score
            try:
                cached_val = self._redis.hget(best_key, "response")
                return cached_val, best_score
            except Exception:
                return None, best_score
